...
C
      CALL SMOOTH(1, NSS, 4, 0.25, SDIST, XMEAN)
      CALL SMOOTH(1, NSS, 4, 0.25, SDIST, RMEAN)
...
C
      VOLFLO = FLOW/DENSMID     
      QLEN   = VOLFLO/(VM_LOC(NS)*SIN(QANGL- PITCH_ANGL(NS)))
     &         /(2*PI*RMEAN(NS))
...
C   SKIP ANY OVERLAPPING POINTS
      ...
      WRITE(6,*) ' N, DXNEXT,DXLAST, PROJN,DISTMIN',
     &             N, DXNEXT,DXLAST, PROJN, DISTMIN
 
      IF(PROJN.GT.DISTMIN) THEN
      NALL = NALL + 1
...
C
     ...
     &            NLE2_ALL(NSTG),NTE2_ALL(NSTG)
      END DO
C
      WRITE(6,*) ' COORDINATES OF THE CONTINUOUS STREAM SURFACE'
      WRITE(6,*) ' NUMBER OF POINTS ON THE CONTINUOUS STREAM SURFACE= ',
     &             NINTPTS
      WRITE(6,*) ' XSURFHUB'
...
C
  ...
  336 CONTINUE 
C
C   SMOOTH THE HUB AND CASING STREAM SURFACE COORDINATES.
C
...
C    FIRST FOR A STATOR
      IF(ROWTYP.EQ.'S') THEN
                BLE   = ATAN( TAN(BLE_DES)*RDESIGNLE/RSECTLE )
                BTE   = ATAN( TAN(BTE_DES)*RDESIGNTE/RSECTTE )
      WRITE(6,*)
...
C
 ...
 1016 FORMAT(8F10.4)
 1017 FORMAT(8F10.4)
 1018 FORMAT(4F10.4,T50,' LEADING AND TRAILING EDGE COORDINATES') 
C
C
      FCENTROID = 1.0
      FTANG     = 0.0
