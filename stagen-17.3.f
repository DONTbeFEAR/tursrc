...
C
      WRITE(6,130) NSECT
  130 FORMAT(/,' DO YOU WANT TO PLOT SECTION NUMBER ', I5)
      READ(5,38) ANS
      IF(ANS.NE.'Y'.AND.ANS.NE.'y') GO TO 37 
C 
C     PLOT THE BLADE 
C 
      CALL SELPLT(9)
      CALL GRFAR(XIN,YSIN,NXPTS,1,'*',1,1)
      CALL PLTAR(XIN,YPIN,NXPTS,1,'*')
      CALL XLABEL('X  COORDINATE ',14)
      CALL YLABEL('Y  COORDINATE ',14)
      CALL TITLE(0.25,0.90,0.75,0.90,0,'BLADE PROFILE',13)
      CALL BRKPLT 
C
